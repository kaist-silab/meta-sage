...
class SML_Model(CVRPModel):
    ...
    def pre_forward(self, reset_state,  target = None, initial=True, test=False, map_feat = None):
    ...
    def forward(self, state, map_feat = None):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, state.load, ninf_mask=state.ninf_mask, map_feat=map_feat)
...
class EAS_CVRP_Decoder(CVRP_Decoder):
    ...
    def forward(self, encoded_last_node, load, ninf_mask, map_feat=None):
