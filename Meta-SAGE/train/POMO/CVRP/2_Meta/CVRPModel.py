...
class EncoderLayer(nn.Module):
    def __init__(self, **model_params):
        ...
        # self.encoder_scale_bias = nn.Sequential(nn.Linear(1, 128, bias=False),
        #                             nn.ReLU(),
    ...
    def forward(self, input1):
        ...
        head_num = self.model_params['head_num']
        # q_bias = self.encoder_scale_bias(torch.ones(input1.shape[0], 1, 1) * (input1.shape[1] -1) / 1000 )
...
def multi_head_attention(q, k, v, rank2_ninf_mask=None, rank3_ninf_mask=None, distance=None):
    ...
    if distance != None:
        score_scaled = score * (1/(distance[:,None,:,:]))
