...
class TSPTester:
    ...
    def run(self):
        ...
        # self.result_log.append('Time', interval)
        # self.result_log.append_all('sage_score', -1 * self.sage_score.mean(dim=1)[0])
    ...
    def _run_sage(self, num_iter=1):
        ...
        checkpoint = torch.load("../1_pretrained_model/symnco_tsp_with_sml.pt", map_location=self.device) # In ICML reported model
    ...
    def _sage_one_batch(self, episode, batch_size, num_iter):
        ...
        distance_mean = distance_matrix.mean(-1, keepdim=True)
        ...
        for iter_i in range(num_iter):
            ...
            # self.sage_score[:, episode:episode+batch_size, iter_i] = sage_score

        return sum_score_curve

    def _initial_pomo_greedy_rollout(self, env, model, distance_matrix, map_feat):
