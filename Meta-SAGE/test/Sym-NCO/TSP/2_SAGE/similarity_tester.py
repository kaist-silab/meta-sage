...
class TSPTester:
    def __init__(self,
        ...
        np_load_500 = np.load('../emb_label/symnco_tsp_ablation_label_500_1000_200.npy')
    ...
    def _run_eas(self, num_iter=1):
        ...
        checkpoint = torch.load("../1_pretrained_model/ICML_confim_no_prior_1000_RL_SL_epoch1.pt", map_location=self.device) #data 5000  SL+RL인데,prior임
        checkpoint = torch.load("../1_pretrained_model/ICML_confim_no_prior_4000_RL_SL_epoch1.pt", map_location=self.device) #data 5000  SL+RL인데,prior임
    ...
    def cal_similarity(self, distance_matrix, map_feat, label):
        ...
        sml_similarity = abs(SML_embedding - label).sum(dim=-1).sum(dim=-1)
        original_similarity = abs(original_embedding - label).sum(dim=-1).sum(dim=-1)
