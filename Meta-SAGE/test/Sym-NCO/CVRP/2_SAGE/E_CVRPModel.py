...
class E_CVRPModel(CVRPModel):
    ...
    def pre_forward(self, reset_state,  map_feat = None, bias = True, return_h_mean=False):
    ...
    def forward_w_incumbent(self, state, best_action, distance):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, state.load, ninf_mask=state.ninf_mask, distance=distance)
    ...
    def forward_w_incumbent_probs(self, state, best_action, distance):
        # best_action.shape = (batch,)
...
class EAS_CVRP_Decoder(CVRP_Decoder):
    ...
    def init_eas_layers_random(self, batch_size):
        ...
        weight2 = torch.torch.distributions.Uniform(low=-init_lim, high=init_lim).sample((batch_size, emb_dim, emb_dim))
        bias2 = torch.torch.distributions.Uniform(low=-init_lim, high=init_lim).sample((batch_size, emb_dim))
        self.eas_W1 = torch.nn.Parameter(weight1)
        self.eas_b1 = torch.nn.Parameter(bias1)
        # self.eas_W2 = torch.nn.Parameter(weight2)
        # self.eas_b2 = torch.nn.Parameter(bias2)
    ...
    def eas_parameters(self):
        # return [[self.eas_W1, self.eas_b1, self.eas_W2, self.eas_b2], [self.alpha]]
        return [self.eas_W1, self.eas_b1, self.eas_W2, self.eas_b2]
    def forward(self, encoded_last_node, load, ninf_mask, distance):
        ...
        if self.use_bias:
            ...
            # Exponential scheduler
            temp = math.exp(math.log(0.3)/200 * (self.iter))
            # temp = 0.994**self.iter
            # temp = 0.99**self.iter
            # temp = 1
            probs = F.softmax(score_masked / temp, dim=2)
                # probs = F.softmax(score_masked / 1, dim=2)
        else:
            score_scaled = score / sqrt_embedding_dim
            score_clipped = logit_clipping * torch.tanh(score_scaled)
            score_masked = score_clipped + ninf_mask
