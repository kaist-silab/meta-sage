...
class CVRPTester:
    def __init__(self,
        ...
        # self.EAS_score = torch.empty(size=(1, num_episode,), device='cpu')
    ...
    def _run_sage(self, num_iter=1):
        ...
        checkpoint = torch.load("../1_pretrained_model/symnco_cvrp_with_sml.pt", map_location=self.device)
    ...
    def _sage_one_batch(self, episode, batch_size, num_iter):
        ...
        map_feat1 = distance_matrix.topk(k=100, dim=-1, largest=False)[0].mean(-1, keepdim=True)
        ...
        for iter_i in range(num_iter):
            ...
            while not done:
                ...
                selected, prob = self.model.forward_w_incumbent_probs(state, best_action, distance)
            ...
            if self.max_val > 0:
                ...
                eas_filename = "cvrplib_result/EAS_X_100_200.txt "
                ours_filename = "cvrplib_result/META_SAGE_X_100_200.txt "
                if iter_i ==  num_iter - 1:
                    if self.run_params['use_bias']:
                        if not os.path.exists(eas_filename):
                            with open(eas_filename, "w") as file:
                                file.write("{}\n".format(-1 * sage_score.mean().item()))
                        else:
                            with open(eas_filename, "a") as file:
                                file.write("{}\n".format(-1 * sage_score.mean().item()))
                    else:
                        if not os.path.exists(ours_filename):
                            with open(ours_filename, "w") as file:
                                file.write("{}\n".format(-1 * sage_score.mean().item()))
                        else:
                            with open(ours_filename, "a") as file:
                                file.write("{}\n".format(-1 * sage_score.mean().item()))
                # print()
