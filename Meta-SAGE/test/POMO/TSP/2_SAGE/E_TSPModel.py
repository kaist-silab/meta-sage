...
class E_TSPModel(TSPModel):
    ...
    def forward(self, state, distance):
    ...
    def forward_w_incumbent(self, state, best_action, distance=None):
    ...
    def forward(self, encoded_last_node, ninf_mask, first_node=None, distance=None):
        ...
        if self.use_bias:
            ...
            if distance == None:
                ...
                temp = alpha
                # beta = (math.log((math.exp(0)-math.exp(1))/200 * self.iter + math.exp(1)))

                # else:
                #     alpha = (1.0*math.exp(math.log(0.3/1.0)/100 * (self.iter -100)))
                #     temp = alpha
                #     # alpha = 0.3
                    # temp = alpha
                # Log scheduler
                # alpha = (math.log((math.exp(0.3)-math.exp(1))/200 * self.iter + math.exp(1)))

                # Linear scheduler
                # alpha = 1 - 0.7/200  * self.iter
            ...
            temp = 1.0*math.exp(math.log(0.3/1.0)/200 * (self.iter))
            ...
            probs = F.softmax(score_masked, dim=2)
            # if self.iter == 200:
            #     probs = F.softmax(score_masked / 0.01, dim=2)
            # else:
            #     probs = F.softmax(score_masked, dim=2)
            # shape: (batch, pomo, problem)
        return probs
