...
def multi_head_attention(q, k, v, rank2_ninf_mask=None, rank3_ninf_mask=None, distance=None):
    ...
    # if distance != None:
    #     distance = (distance - distance.min(dim=-1, keepdim=True)[0]) / distance.max(dim=-1, keepdim=True)[0]
    #     score_scaled = score_scaled - distance[:,None,:,:].repeat(1, head_num, 1, 1)
...
def multi_head_attention1(q, k, v, rank2_ninf_mask=None, rank3_ninf_mask=None, iter=None):
    # q shape: (batch, head_num, n, key_dim)   : n can be either 1 or PROBLEM_SIZE
    # k,v shape: (batch, head_num, problem, key_dim)
    # rank2_ninf_mask.shape: (batch, problem)
    # rank3_ninf_mask.shape: (batch, group, problem)
    ...
    weights = nn.Softmax(dim=3)(score_scaled/(1.0 - 0.80/200 * iter))
    # shape: (batch, head_num, n, problem)
